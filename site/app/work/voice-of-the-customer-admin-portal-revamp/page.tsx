...
export default function VoiceOfCustomerAdminPortalPage() {
  return <main className="case-study feedback-case-study">
    ...
    <CaseStudySection id="challenge" eyebrow="02 · Challenge" title="The interface was only the visible symptom" tone="pink"><ContentBlockRenderer blocks={vocContent.challenge.slice(0, 2)} renderMedia={renderMedia} /><InsightGrid mode="insights" groups={[
      <><h4>Conflicting product definitions</h4><p>Terms such as “public program” and “private program” meant different things in the platform than they did within Splunk’s product organization, creating confusion and SME dependency.</p></>,
      <><h4>A black-box creation experience</h4><p>Program owners entered content without knowing how it would appear to customers, leading to hesitation and increased support needs.</p></>,
      <><h4>Workflows that ignored real dependencies</h4><p>Creating a program required materials from Legal, Engineering, and other teams, yet users could not save incomplete programs or see upcoming requirements.</p></>,
      <><h4>Limited interface scalability</h4><p>Modal-heavy patterns constrained complex forms, navigation, and future capabilities.</p></>,
    ]} /><WorkflowQuestion>This required more than a visual refresh: the product model, information architecture, technical capabilities, and interaction patterns all needed reconsideration.</WorkflowQuestion></CaseStudySection>
