...
const worker = {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    if (PROTECTED_PROJECTS.has(url.pathname.replace(/\/$/, ""))) {
      const token = await accessToken();
      const cookies = Object.fromEntries((request.headers.get("cookie") ?? "").split(";").map((part) => part.trim().split("=", 2)));
      if (cookies[ACCESS_COOKIE] !== token) {
        if (request.method === "POST") {
          const form = await request.formData();
          if (form.get("password") === PROJECT_PASSWORD) {
            return new Response(null, { status: 303, headers: { location: `${url.pathname}${url.search}`, "set-cookie": `${ACCESS_COOKIE}=${token}; Path=/; HttpOnly; Secure; SameSite=Lax` } });
          }
